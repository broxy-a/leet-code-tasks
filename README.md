...
В этой задаче требуется пройти по бинарному дереву сделать это надо с помощью метода inorder traversal —  то есть пройтись по каждому узлу дерева в порядке: левый — корень — правый
...
🔹 Основные моменты:
    ...
    📌 Рекурсивная функция сначала двигается по левому поддереву до дна, затем по правому.
    📌 Сложность O(n) - т.к каждый узел посещается один раз.
    📌 Результат записывается в список.