...
Нужно слить два отсортированных массива в один, при этом не создавая новый изменяя nums1.

nums1[1,2,3,0,0,0]; nums2[2,5,6] -> nums1[1,2,2,3,5,6]
nums1[1]; nums2[] -> nums1[1]

🔹 Основные моменты:
    📌 Используются указатели, для движения внутри массива i, j, k
    📌 Сложность O(m + n).
    📌 Обработка случаев: когда элемент nums2 больше, когда элемент nums1 больше и в случае равенства значений.
    📌 Обработка остатка nums2
