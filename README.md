...
Даны две строки, нужно найти с какого индекс начинается подстрока needle в строке haystack.

haystack = "leetcode", needle = "leeto" -> index = 0
haystakc = "sadbutsad", needle = "sad" -> index = -1

🔹 Основные моменты:
    📌 Цикл проходит по длинне строк от haystack до needle, что бы он "знал" границы
    📌 Если подстрока haystack с i равна с needle, то вернем i как индекс вхожденияю
    📌 Если ничего не найдено, то вернуть -1 (условие)
