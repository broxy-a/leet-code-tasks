...
🔹 Основные моменты:
    📌 Временная сложность у задачи, O(n). В лучшем случае O(log n).
    📌 Используется рекурсивная проверка, оба поддерева должны быть идентичны
    📌 Идентичность определяется: Расположением узлов и их значениями.

101. Symmetric Tree

В данной задаче нужно проверить симметричность деревьев
...
🔹 Основные моменты:
    📌 Симметриность - это зеркальное отражение, а не идентичность поддеревьев.
    📌 Рекурсивное сравнение двух поддеревьев isMirror(left, right).
    📌 Слонжость задачи O(n), n - количество узлов. Средняя слонжость O(log n).
    📌 Расписаны 3 базовых случая для рекурсии.

104. Maximum Depth of Binary Tree

В данной задаче нужно написать функцию которая будет возвращать максимальную глубину дерева.
...
🔹 Основные моменты:
    ...
    📌 +1 в основной логике означает что мы учитываем еще текущий узел.

112 Path Sum

В данной задаче нужно найти пусть от корня до листа в сумме которых получится искомое число.
...
🔹 Основные моменты:
    ...
    📌 Временная сложность: O(n), Пространственная сложность: O(h)
